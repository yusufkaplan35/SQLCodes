...
INSERT INTO calisanlar VALUES('10007', 'Raphael', '', '2018-04-14'); --maas: integer olmalı
...
INSERT INTO adresler VALUES('10012','Taş Sok', '30.Cad.','Konya'); --10012 id li çalışan yok o yüzden kabul etmiyor
...
--ismi Ahmet Ran veya Veli Han olan kaydı silelim
DELETE FROM ogrenciler1 WHERE isim='Ahmet Ran' OR isim='Veli Han'


--16- tablodaki tüm kayıtları sil DELETE FROM .. : koşul belirtmezsek tablodaki tüm kayıtlar silinir
SELECT * FROM students;
--students tablosundaki tüm kayıtları silelim
DELETE FROM students


--16- Tablodaki tüm kayıtları silme: TRUNCATE TABLE ..
SELECT * FROM doctors;

--doctors tablosundaki tüm kayıtları silme
...
DELETE FROM calisanlar; --bazı kayıtlar adresler tablosunda referans alındığından silemez
DELETE FROM calisanlar WHERE id='10003';--bazı kayıtlar adresler tablosunda referans alındığından silemez

DELETE FROM adresler WHERE adres_id='10003';-- referans alınan kayıt silindi, ilişki koparıldı
DELETE FROM calisanlar WHERE id='10003'; -- ilişki koparıldığı için, artık referans olmadığı için siler

DELETE FROM adresler; --tüm ilişki koparıldı 
DELETE FROM calisanlar; -- artık hiçbir kayıt referans alınmıyor
...
--parenttan referanslı kayıt silmek için aşamalı (cascade) silme yapmalıydık
-- on delete cascade özelliği==> önce notlar tablosundan referans olan kaydı siler
--sonra parenttan referens kaldırılan kaydı siler.

DELETE FROM talebeler WHERE id=126;

DELETE FROM talebeler; --notlar tablosundan da sadece referanslı olan kayıtları otomatik olarak siler


-- 19- Tabloyu silme: tabloyu SCHEMAdan silme
-- sairler tablosunu silelim
DROP TABLE sairler; --DDL
-- talebeler tablosunu silelim
DROP TABLE talebeler; --ilişki hala tanımlı olduğu için silemez
DROP TABLE talebeler CASCADE; -- önce bağlantıyı kesip sonra silecek

-- talebeler1 tablosunu silelim
DROP TABLE IF EXISTS talebeler1; --hata almamak için EĞER VARSA SİL yapmış olduk 
...
SELECT *
FROM musteriler
WHERE urun_id BETWEEN 20 AND 40; -- 20 VE 40 DAHİL

--Müşteriler tablosunda urun_id 20 den kucuk 30(dahil değil) buyuk olan urunlerin tum bilgilerini listeleyiniz
SELECT *
FROM musteriler
...
SELECT *
FROM musteriler
WHERE urun_id NOT 20 BETWEEN 30;
...
--calisanlar3 tablosunda max maasi goruntuleyiniz
SELECT MAX(maas) FROM calisanlar3;

--calisanlar3 tablosunda min maasi goruntuleyiniz
SELECT MIN(maas) FROM calisanlar3;

--calisanlar3 tablosunda toplam maasi goruntuleyiniz
SELECT SUM(maas) FROM calisanlar3;

--calisanlar3 tablosunda ortalama maasi goruntuleyiniz
SELECT AVG(maas) FROM calisanlar3;
SELECT ROUND(AVG(maas),2) FROM calisanlar3;

--calisanlar3 tablosunda kayıt sayısını goruntuleyiniz
SELECT COUNT(*) FROM calisanlar3;
SELECT COUNT(maas) FROM calisanlar3;

--calisanlar3 tablosunda maası 2500 olanların sayısını goruntuleyiniz
